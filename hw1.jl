...
student = (name = "Lesha Seletskiy", kerberos_id = "jazz")
...
function my_sum(xs)
	# your code here!
	sum=0
	for i in xs # pfc: nice, this is cleaner than using 1:length() and prob. faster?
		sum+=i
	end
	return sum
end
...
function demean(xs)
	# your code here!
	x=copy(xs)
	x.-=mean(xs) # pfc: riiight, this isn't the approach i took, i like yours more!
	return x
end
...
function create_bar()
	# your code here!
	x=zeros(100)
	x[50:60].=1 # pfc: small typo here, range should be 40:60
	return x
end
...
function get_reds(image::AbstractMatrix)
	return get_red.(image) # broadcasting again... :D
end
...
# ╔═╡ acc296e8-754f-11eb-12c0-0f280cb25045
function get_greens(image::AbstractMatrix)
	return get_green.(image)
end

# ╔═╡ caf702e8-754f-11eb-0b8b-650e13167130
function get_blue(pixel::AbstractRGB)
	return pixel.b
end

# ╔═╡ d382192a-754f-11eb-14cd-b98d87cd5ad1
...
function mean_color(image)
	# your code here!
	r=sum(i->i, get_reds(image)) # pfc: interested to understand what you're doing
	g=sum(i->i, get_greens(image)) # here with sum(i->i, ...)
	b=sum(i->i, get_blues(image))
	t=length(image)
	return RGB(r,g,b)/t
end
...
# ╔═╡ 4345198a-7b28-11eb-3359-796989bd6069
sum
...
The image is unrecognisable with intensity ...
	Becuase with the intensity set to one, you often get small noise values which leave the image recognizable, with higher intensities small noise values become less frequent.
"""

# pfc: you didn't answer the above question
...
function custom_filter(pixel::AbstractRGB)
	return RGB(pixel.r^.5, pixel.g^2, pixel.b^3)
end
