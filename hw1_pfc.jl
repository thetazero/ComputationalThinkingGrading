...
function get_reds(image::AbstractMatrix)
	#your code here!
	x, y = size(image)
	reds = zeros(length(image))
	for i in 1:length(image)
		reds[i] = get_red(image[i])
	end
	reds = reshape(reds, (x,y))
	#reds = get_red.(image)
	return reds
end
...
philip_reds = get_reds(philip_head)

# ╔═╡ d31a06fe-77fd-11eb-0519-17a91c893fa3
value_as_color.(philip_reds)
...
# ╔═╡ deeeb528-77fe-11eb-1633-57ad6993e604
function get_greens(image)
	return get_green.(image)
end

# ╔═╡ ef57a40e-77fe-11eb-1f8d-51a3855b5852
function get_blue(pixel::AbstractRGB)
	return pixel.b
end

# ╔═╡ fe7cac62-77fe-11eb-116e-b963a57cfc76
...
function mean_color(image)
	# your code here!
	reds = mean(get_reds(image))
	greens = mean(get_greens(image))
	blues = mean(get_blues(image))
	n = 3
	# mean_color = reds + greens + blues / n
...
# ╔═╡ 18b69d7c-7807-11eb-0df1-ab5d501c8ec2
s=1.0

# ╔═╡ 1477c948-7807-11eb-3bbb-438327a74b26
rand([-s;s])*rand()

# ╔═╡ 8440839c-780b-11eb-049c-f1a011137f1d
rand([-s;s])
...
function noisify(color::AbstractRGB, s)
	# your code here!
	red = noisify(color.r,s)
	green = noisify(color.g,s)
	blue = noisify(color.b,s)
	return RGB(red,green,blue)
end
...
function noisify(image::AbstractMatrix, s)
	# your code here!
	# noisy_image = noisify.(image)
	noisy_image = [noisify.(pixel,s) for pixel=1:length(image)]
	return noisy_image
end

# ╔═╡ 1ea53f41-b791-40e2-a0f8-04e13d856829
maximum([noisify.(0.9, 0.1) for _ in 1:1000]) # edit this test case!
...
The image is unrecognisable with intensity 4.5-5
"""
...
function custom_filter(pixel::AbstractRGB)#,s::Float64)
	
	# your code here!
	# red = pixel.r + s
	# blue = pixel.b + s
	# green = pixel.g + s
	return RGB(pixel.r+0.5,pixel.g+0.5,pixel.b+0.5)
end

# ╔═╡ 9e5a08dd-332a-486b-94ab-15c49e72e522
function custom_filter(image::AbstractMatrix)
	x, y = size(image)
	# for i in 1:4:length(x), j in 1:length(y)
		# image[i,j] = custom_filter(image[i,j])
